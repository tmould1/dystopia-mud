...
Totems are mob entities (like Artificer turrets) with HP that can be attacked and destroyed:

| Define | Vnum | Purpose |
|--------|------|---------|
| `VNUM_SHAMAN_TOTEM` | 33596 | Totem entity (placed in room via `create_mobile()`) |

Totem stats are set programmatically at spawn time. The mob template in `classeq.db` is minimal.
...
| Label | Source | Notes |
|-------|--------|-------|
| Spirit Tether | STAT_RAGE | Current tether position (0-100) |
| Active Totem | Custom | Which totem is active (Ward/Wrath/Spirit/None) |
| Totems | Custom | Totem training level (0-3) |
| Spirits | Custom | Spirit training level (0-3) |
| Communion | Custom | Communion training level (0-3) |
...
| soul_link_type | ch->pcdata->powers[7] | Soul Link type (0=enemy via ch->fighting) |
| active_totem | ch->pcdata->powers[8] | Active totem type (0=none, 1=ward, 2=wrath, 3=spirit) |
...
| wrathtotem_cd | ch->pcdata->powers[14] | Wrath Totem cooldown ticks remaining |
| spirittotem_cd | ch->pcdata->powers[15] | Spirit Totem cooldown ticks remaining |
| spiritwalk_cd | ch->pcdata->powers[16] | Spirit Walk cooldown ticks remaining |
| soullink_cd | ch->pcdata->powers[17] | Soul Link cooldown ticks remaining |
| spiritbolt_cd | ch->pcdata->powers[18] | Spirit Bolt cooldown ticks remaining |
