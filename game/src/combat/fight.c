...
int number_attacks( CHAR_DATA *ch, CHAR_DATA *victim ) {
	...
	if ( IS_NPC( ch ) ) {
		...
		if ( ch->pIndexData->hitsizedice > 20 )
			count += 20;
		else
			count += ch->pIndexData->hitsizedice;
		return count;
	}
	if ( IS_NPC( victim ) ) {
...
void one_hit( CHAR_DATA *ch, CHAR_DATA *victim, int dt, int handtype ) {
	...
	if ( IS_NPC( ch ) && ch->pIndexData->hitplus > 3 )
		dam *= 3;
	else if ( IS_NPC( ch ) && ch->pIndexData->hitplus > 0 )
		dam *= ch->pIndexData->hitplus;
...
void update_damcap( CHAR_DATA *ch, CHAR_DATA *victim ) {
	...
	if ( !IS_NPC( ch ) ) {
		...
		if ( IS_CLASS( ch, CLASS_SHAPESHIFTER ) ) {
			max_dam += 800;
			if ( ch->pcdata->powers[SHAPE_FORM] == TIGER_FORM )
				max_dam += ch->pcdata->powers[TIGER_LEVEL] * 325;
			else if ( ch->pcdata->powers[SHAPE_FORM] == HYDRA_FORM )
				max_dam += ch->pcdata->powers[HYDRA_LEVEL] * 350;
			else if ( ch->pcdata->powers[SHAPE_FORM] == BULL_FORM )
				max_dam += ch->pcdata->powers[BULL_LEVEL] * 300;
			else if ( ch->pcdata->powers[SHAPE_FORM] == FAERIE_FORM )
				max_dam += ch->pcdata->powers[FAERIE_LEVEL] * 275;
		}
		if ( IS_CLASS( ch, CLASS_DROW ) ) {
			max_dam += 500;
...
void hurt_person( CHAR_DATA *ch, CHAR_DATA *victim, int dam ) {
	...
	if ( !IS_NPC( victim ) && IS_SET( victim->newbits, NEW_CLOAK ) && victim->hit < 1 ) {
		...
		if ( IS_CLASS( victim, CLASS_MONK ) )
			...
			send_to_char( "your cloak of death prevents the destruction of your body.\n\r", victim );
...
bool check_parry( CHAR_DATA *ch, CHAR_DATA *victim, int dt ) {
	...
	if ( IS_NPC( victim ) )
		...
		obj = NULL;
		claws = TRUE;
	} else if ( IS_CLASS( victim, CLASS_SHAPESHIFTER ) && get_eq_char( victim, WEAR_WIELD ) == NULL && get_eq_char( victim, WEAR_HOLD ) == NULL ) {
		obj = NULL;
		claws = TRUE;
	} else {
	...
	if ( !IS_NPC( ch ) ) {
		...
		if ( IS_CLASS( ch, CLASS_SHAPESHIFTER ) ) {
			if ( ch->pcdata->powers[SHAPE_FORM] == TIGER_FORM )
				...
				chance -= (int) ( ch->pcdata->powers[BULL_LEVEL] * 7.5 );
			else if ( ch->pcdata->powers[SHAPE_FORM] == HYDRA_FORM )
	...
	if ( !IS_NPC( victim ) ) {
		...
		if ( IS_CLASS( victim, CLASS_SHAPESHIFTER ) ) {
			if ( victim->pcdata->powers[SHAPE_FORM] == TIGER_FORM )
				chance += victim->pcdata->powers[TIGER_LEVEL] * 6;
			else if ( victim->pcdata->powers[SHAPE_FORM] == BULL_FORM )
				chance += victim->pcdata->powers[BULL_LEVEL] * 7;
			else if ( victim->pcdata->powers[SHAPE_FORM] == HYDRA_FORM )
	...
	if ( !IS_NPC( ch ) && IS_CLASS( ch, CLASS_SHAPESHIFTER ) ) {
		if ( ch->pcdata->powers[SHAPE_FORM] == TIGER_FORM )
			chance -= ch->pcdata->powers[TIGER_LEVEL] * 4;
		else if ( ch->pcdata->powers[SHAPE_FORM] == BULL_FORM )
			chance -= ch->pcdata->powers[BULL_LEVEL] * 3;
		else if ( ch->pcdata->powers[SHAPE_FORM] == HYDRA_FORM )
			chance -= ch->pcdata->powers[HYDRA_LEVEL] * 3;
		else if ( ch->pcdata->powers[SHAPE_FORM] == FAERIE_FORM )
			chance -= ch->pcdata->powers[FAERIE_LEVEL] * 2;
		else
			chance -= 2;
	}
	if ( IS_CLASS( ch, CLASS_TANARRI ) && !IS_NPC( ch ) ) {
	...
	if ( !IS_NPC( victim ) && IS_CLASS( victim, CLASS_SHAPESHIFTER ) ) {
		if ( victim->pcdata->powers[SHAPE_FORM] == TIGER_FORM )
			chance += victim->pcdata->powers[TIGER_LEVEL] * 2;
		else if ( victim->pcdata->powers[SHAPE_FORM] == BULL_FORM )
			chance += victim->pcdata->powers[BULL_LEVEL] * 3;
		else if ( victim->pcdata->powers[SHAPE_FORM] == HYDRA_FORM )
			chance += victim->pcdata->powers[HYDRA_LEVEL] * 3;
		else if ( victim->pcdata->powers[SHAPE_FORM] == FAERIE_FORM )
			chance += victim->pcdata->powers[FAERIE_LEVEL] * 6;
		else
			chance += 2;
	}
	...
	if ( !IS_NPC( ch ) && IS_NPC( victim ) && IS_SET( ch->newbits, NEW_FIGHTDANCE ) && IS_CLASS( ch, CLASS_DROW ) && number_percent() >= 50 && ( ( ( obj = get_eq_char( ch, WEAR_WIELD ) ) != NULL && ( obj->value[3] == 4 || obj->value[3] == 11 ) ) || ( ( obj = get_eq_char( ch, WEAR_HOLD ) ) != NULL && ( obj->value[3] == 4 || obj->value[3] == 11 ) ) ) ) {
...
int xp_compute( CHAR_DATA *gch, CHAR_DATA *victim ) {
	...
	if ( !IS_NPC( gch ) && xp > 499 && gch->pcdata->disc_points != 999 && gch->pcdata->disc_research != -1 ) {
		...
		if ( !IS_SET( gch->act, PLR_BRIEF4 ) )
			if ( victim->level > 200 && gch->pcdata->disc_points != 999 ) {
				stc( "#RYou gained a discipline point.#n\n\r", gch );
				gain_disc_points( gch, 1 );
			}
		if ( !IS_SET( gch->act, PLR_BRIEF4 ) )
			if ( victim->level > 400 && gch->pcdata->disc_points != 999 ) {
				stc( "#RYou gained a discipline point.#n\n\r", gch );
				gain_disc_points( gch, 1 );
			}
		xp -= 500;
...
void dam_message( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt ) {
	...
	if ( ( victim->hit - dam > damp ) || ( dt >= 0 && dt < MAX_SKILL ) ||
		...
		if ( !( IS_SET( ch->act, PLR_BRIEF2 ) && ( dam == 0 || dt == skill_lookup( "lightning bolt" ) || dt == skill_lookup( "acid blast" ) || dt == skill_lookup( "chill touch" ) || dt == skill_lookup( "fireball" ) ) ) )
			act( buf2, ch, NULL, victim, TO_CHAR );
		if ( !( IS_SET( victim->act, PLR_BRIEF2 ) && ( dam == 0 || dt == skill_lookup( "lightning bolt" ) || dt == skill_lookup( "acid blast" ) || dt == skill_lookup( "chill touch" ) || dt == skill_lookup( "fireball" ) ) ) )
	...
	if ( !strcmp( attack, "slash" ) || !strcmp( attack, "slice" ) ) {
	...
	} else if ( !strcmp( attack, "stab" ) || !strcmp( attack, "pierce" ) ) {
		damp = number_range( 1, 5 );
		if ( damp == 1 ) {
	...
	} else if ( !IS_NPC( ch ) && ( !strcmp( attack, "bite" ) || IS_VAMPAFF( ch, VAM_FANGS ) ) ) {
...
void do_backstab( CHAR_DATA *ch, char *argument ) {
	...
	if ( ( ch->pcdata->rank == BELT_SEVEN || ch->pcdata->rank == BELT_EIGHT || ch->pcdata->rank == BELT_NINE || ch->pcdata->rank == BELT_TEN ) && number_range( 1, 3 ) == 1 )
		multi_hit( ch, victim, gsn_backstab );
...
void do_garotte( CHAR_DATA *ch, char *argument ) {
	...
	if ( is_safe( ch, victim ) ) return;
	if ( ( ( obj = get_eq_char( ch, WEAR_WIELD ) ) == NULL || obj->value[3] != 4 ) && ( ( obj = get_eq_char( ch, WEAR_HOLD ) ) == NULL || obj->value[3] != 4 ) ) {
...
void do_dark_garotte( CHAR_DATA *ch, char *argument ) {
	...
	if ( !IS_CLASS( ch, CLASS_DROW ) || !IS_SET( ch->pcdata->powers[1], DPOWER_GAROTTE ) || !IS_SET( ch->pcdata->powers[1], DPOWER_DGAROTTE ) ) {
		send_to_char( "Huh?\n\r", ch );
		return;
	}
