...
bool spec_gremlin_original( CHAR_DATA *ch ) {
	...
	LIST_FOR_EACH_SAFE(object, object_next, &ch->in_room->objects, OBJ_DATA, room_node) {
		...
		if ( object->item_type == ITEM_FOOD || object->item_type == ITEM_TRASH || object->item_type == ITEM_CORPSE_NPC ) {
		...
		LIST_FOR_EACH( victim, &g_characters, CHAR_DATA, char_node ) {
			...
			if ( victim->in_room == ch->in_room ) {
				...
				if ( char_ac( victim ) - 50 > char_ac( ch ) )
					...
					consider += 1;
				if ( char_hitroll( victim ) + 10 < char_hitroll( ch ) )
					consider += 1;
				else if ( char_hitroll( victim ) - 10 > char_hitroll( ch ) )
					consider -= 1;
				if ( char_damroll( victim ) + 10 < char_damroll( ch ) )
					consider += 1;
				else if ( char_damroll( victim ) - 10 > char_damroll( ch ) )
					consider -= 1;
				switch ( consider ) {
				default:
					break;
