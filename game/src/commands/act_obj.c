...
void do_eat( CHAR_DATA *ch, char *argument ) {
	...
	if ( !IS_IMMORTAL( ch ) ) {
		...
		if ( obj->item_type != ITEM_FOOD && obj->item_type != ITEM_PILL &&
			obj->item_type != ITEM_EGG && obj->item_type != ITEM_QUEST && obj->item_type != ITEM_DRAGONGEM && obj->item_type != ITEM_DTOKEN ) {
...
void do_steal( CHAR_DATA *ch, char *argument ) {
	...
	if ( ( ch->level + number_range( 1, 20 ) < victim->level ) || ( !IS_NPC( ch ) && !IS_NPC( victim ) && ch->level < 3 ) || ( !IS_NPC( ch ) && !IS_NPC( victim ) && victim->level < 3 ) || ( victim->position == POS_FIGHTING ) || ( !IS_NPC( victim ) && IS_IMMUNE( victim, IMM_STEAL ) ) || ( !IS_NPC( victim ) && IS_IMMORTAL( victim ) ) || ( !IS_NPC( ch ) && percent > ch->pcdata->learned[gsn_steal] ) ) {
		/*
		 * Failure.
		 */
		send_to_char( "Oops.\n\r", ch );
...
void do_activate( CHAR_DATA *ch, char *argument ) {
	...
	if ( IS_SET( obj->spectype, SITEM_TARGET ) ) {
		...
		victim = ch;
	if ( obj->chpoweruse != NULL && obj->chpoweruse[0] != '\0' && str_cmp( obj->chpoweruse, "(null)" ) )
		kavitem( obj->chpoweruse, ch, obj, NULL, TO_CHAR );
	if ( obj->victpoweruse != NULL && obj->victpoweruse[0] != '\0' && str_cmp( obj->victpoweruse, "(null)" ) )
	...
	if ( IS_SET( obj->spectype, SITEM_SPELL ) ) {
		...
		if ( obj->chpoweron != NULL && obj->chpoweron[0] != '\0' && str_cmp( obj->chpoweron, "(null)" ) )
			kavitem( obj->chpoweron, ch, obj, NULL, TO_CHAR );
		if ( obj->victpoweron != NULL && obj->victpoweron[0] != '\0' && str_cmp( obj->victpoweron, "(null)" ) )
		...
		do_look( ch, "auto" );
		if ( obj->chpoweroff != NULL && obj->chpoweroff[0] != '\0' && str_cmp( obj->chpoweroff, "(null)" ) )
			kavitem( obj->chpoweroff, ch, obj, NULL, TO_CHAR );
		if ( obj->victpoweroff != NULL && obj->victpoweroff[0] != '\0' && str_cmp( obj->victpoweroff, "(null)" ) )
			kavitem( obj->victpoweroff, ch, obj, NULL, TO_ROOM );
		if ( !IS_SET( obj->quest, QUEST_ARTIFACT ) && ( IS_SET( ch->in_room->room_flags, ROOM_NO_TELEPORT ) || ch->in_room->vnum == 30000 || IS_SET( ch->in_room->room_flags, ROOM_ASTRAL ) ) && CAN_WEAR( obj, ITEM_TAKE ) ) {
