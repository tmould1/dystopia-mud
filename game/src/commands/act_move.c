	...
	if ( !IS_NPC( ch ) ) {
		...
		if ( in_room->sector_type == SECT_AIR || to_room->sector_type == SECT_AIR ) {
			if ( !IS_AFFECTED( ch, AFF_FLYING ) &&
				( !IS_NPC( ch ) && ( !IS_CLASS( ch, CLASS_DROW ) || !IS_SET( ch->pcdata->powers[1], DPOWER_LEVITATION ) ) ) &&
...
ROOM_INDEX_DATA *get_random_room( CHAR_DATA *ch ) {
	...
	for ( ;; ) {
		...
		if ( room != NULL )
			if ( !IS_SET( room->room_flags, ROOM_PRIVATE ) && !IS_SET( room->room_flags, ROOM_SOLITARY ) && !IS_SET( room->room_flags, ROOM_SAFE ) )
				break;
	}
...
ROOM_INDEX_DATA *get_rand_room() {
	...
	for ( ;; ) {
		room = get_room_index( number_range( 0, 32000 ) );
		if ( room != NULL )
			if ( !IS_SET( room->room_flags, ROOM_PRIVATE ) && !IS_SET( room->room_flags, ROOM_SOLITARY ) && !IS_SET( room->room_flags, ROOM_SAFE ) && room->vnum > 300 )
				break;
	}
...
ROOM_INDEX_DATA *get_treemeld_room() {
	...
	for ( ;; ) {
		room = get_room_index( number_range( 0, 32000 ) );
		if ( room != NULL && room->sector_type == SECT_FOREST ) break;
	}
...
ROOM_INDEX_DATA *get_rand_room_by_sect( int sect ) {
	...
	for ( ;; ) {
		room = get_room_index( number_range( 0, 32000 ) );
		if ( room != NULL && room->sector_type == sect ) break;
	}
...
void do_meditate( CHAR_DATA *ch, char *argument ) {
	if ( IS_NPC( ch ) || ( !IS_CLASS( ch, CLASS_MAGE ) && !IS_CLASS( ch, CLASS_MONK ) && !IS_CLASS( ch, CLASS_NINJA ) && !IS_CLASS( ch, CLASS_DROW ) && !IS_CLASS( ch, CLASS_LICH ) ) ) {
...
void do_train( CHAR_DATA *ch, char *argument ) {
	...
	if ( !str_cmp( arg1, "str" ) )
		...
		!str_cmp( arg1, "steal" ) || !str_cmp( arg1, "stab" ) )
		is_ok = TRUE;
		...
		is_ok = TRUE;
	else if ( !str_cmp( arg1, "control" ) && IS_CLASS( ch, CLASS_VAMPIRE ) && ch->beast > 0 )
		is_ok = TRUE;
	...
	if ( !str_cmp( arg1, "str" ) ) {
		...
		return;
	} else if ( !str_cmp( arg1, "methuselah" ) && IS_CLASS( ch, CLASS_VAMPIRE ) && ( ch->pcdata->rank == AGE_ELDER ) ) {
		...
		return;
	} else if ( !str_cmp( arg1, "lamagra" ) && IS_CLASS( ch, CLASS_VAMPIRE ) && ( ch->pcdata->rank == AGE_METHUSELAH ) ) {
		if ( ch->exp < lamagra ) {
		...
		return;
	} else if ( !str_cmp( arg1, "trueblood" ) && IS_CLASS( ch, CLASS_VAMPIRE ) && ( ch->pcdata->rank == AGE_LA_MAGRA ) ) {
		if ( ch->exp < trueblood ) {
	...
	else if ( !str_cmp( arg1, "sunlight" ) && !IS_IMMUNE( ch, IMM_SUNLIGHT ) && IS_CLASS( ch, CLASS_VAMPIRE ) ) {
		if ( ch->exp < 1000000 ) {
	...
	else if ( !str_cmp( arg1, "belt1" ) && IS_CLASS( ch, CLASS_NINJA ) && ( ch->pcdata->rank < BELT_ONE ) ) {
		if ( ch->exp < belt1 ) {
		...
		return;
	} else if ( !str_cmp( arg1, "belt2" ) && IS_CLASS( ch, CLASS_NINJA ) && ( ch->pcdata->rank == BELT_ONE ) ) {
		if ( ch->exp < belt2 ) {
		...
		return;
	} else if ( !str_cmp( arg1, "belt3" ) && IS_CLASS( ch, CLASS_NINJA ) && ( ch->pcdata->rank == BELT_TWO ) ) {
		if ( ch->exp < belt3 ) {
		...
		return;
	} else if ( !str_cmp( arg1, "belt4" ) && IS_CLASS( ch, CLASS_NINJA ) && ( ch->pcdata->rank == BELT_THREE ) ) {
		if ( ch->exp < belt4 ) {
		...
		return;
	} else if ( !str_cmp( arg1, "belt5" ) && IS_CLASS( ch, CLASS_NINJA ) && ( ch->pcdata->rank == BELT_FOUR ) ) {
		if ( ch->exp < belt5 ) {
		...
		return;
	} else if ( !str_cmp( arg1, "belt6" ) && IS_CLASS( ch, CLASS_NINJA ) && ( ch->pcdata->rank == BELT_FIVE ) ) {
		if ( ch->exp < belt6 ) {
		...
		return;
	} else if ( !str_cmp( arg1, "belt7" ) && IS_CLASS( ch, CLASS_NINJA ) && ( ch->pcdata->rank == BELT_SIX ) ) {
		if ( ch->exp < belt7 ) {
		...
		return;
	} else if ( !str_cmp( arg1, "belt8" ) && IS_CLASS( ch, CLASS_NINJA ) && ( ch->pcdata->rank == BELT_SEVEN ) ) {
		if ( ch->exp < belt8 ) {
		...
		return;
	} else if ( !str_cmp( arg1, "belt9" ) && IS_CLASS( ch, CLASS_NINJA ) && ( ch->pcdata->rank == BELT_EIGHT ) ) {
		if ( ch->exp < belt9 ) {
		...
		return;
	} else if ( !str_cmp( arg1, "belt10" ) && IS_CLASS( ch, CLASS_NINJA ) && ( ch->pcdata->rank == BELT_NINE ) ) {
		if ( ch->exp < belt10 ) {
