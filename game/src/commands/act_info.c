...
const char *get_room_tint_color( ROOM_INDEX_DATA *room ) {
	...
	indoors = ( room->sector_type == SECT_INSIDE || IS_SET( room->room_flags, ROOM_INDOORS ) );
	hour = time_info.hour;
...
int char_hitroll( CHAR_DATA *ch ) {
	...
	if ( !IS_NPC( ch ) && IS_CLASS( ch, CLASS_NINJA ) && ch->pcdata->powers[NPOWER_CHIKYU] >= 6 && ch->pcdata->powers[HARA_KIRI] > 0 )
		value += 100;
		...
		if ( fMxp && prgpObjShow[iShow] != NULL ) {
			...
			if ( prgpObjShow[iShow]->in_obj != NULL ) {
				...
				send_to_char( mxp_container_item_link( prgpObjShow[iShow],
								  prgpObjShow[iShow]->in_obj, ch, prgpstrDesc[iShow] ),
					ch );
			} else {
...
void show_char_to_char_0( CHAR_DATA *victim, CHAR_DATA *ch ) {
	...
	/* Class titles (from game.db class_display table) */
	if ( !IS_NPC( victim ) && !IS_NPC( ch ) && IS_HERO( victim ) && victim->class > 0 ) {
		/* Special case: Vampire hides class when polymorphed */
		if ( !IS_CLASS( victim, CLASS_VAMPIRE ) || !IS_AFFECTED( victim, AFF_POLYMORPH ) ) {
			const char *look_disp = class_get_look_display( victim->class );
			if ( look_disp && look_disp[0] != '\0' ) {
				CLASS_DISPLAY_DATA *cd = class_display_get( victim->class );
				const char *class_name = cd ? cd->name : "Unknown";
				strcat( prefix_buf, mxp_aura_tag( ch, look_disp, class_name, -1 ) );
			}
		}
	}
	...
	if ( ( mount = victim->mount ) != NULL && victim->mounted == IS_RIDING ) {
		...
		strcat( desc_buf, " is hovering here" );
	else if ( victim->position == POS_STANDING && !IS_NPC( victim ) && IS_CLASS( victim, CLASS_DROW ) && IS_SET( victim->pcdata->powers[1], DPOWER_LEVITATION ) )
		strcat( desc_buf, " is hovering here" );
	else {
		switch ( victim->position ) {
		case POS_DEAD:
			strcat( desc_buf, " is DEAD!!" );
			break;
		case POS_MORTAL:
			strcat( desc_buf, " is #Rmortally wounded#n." );
			break;
		case POS_INCAP:
			strcat( desc_buf, " is #rincapacitated#n." );
			break;
		case POS_STUNNED:
			strcat( desc_buf, " is lying here #Cstunned#n." );
			break;
		case POS_SLEEPING:
			strcat( desc_buf, " is sleeping here." );
			break;
		case POS_RESTING:
			strcat( desc_buf, " is resting here." );
			break;
		case POS_MEDITATING:
			strcat( desc_buf, " is meditating here." );
			break;
		case POS_SITTING:
...
void show_char_to_char( CHAR_DATA *list, CHAR_DATA *ch ) {
	...
	for ( rch = list; rch != NULL; rch = rch->next_in_room ) {
		...
		else if ( room_is_dark( ch->in_room ) && ( IS_AFFECTED( rch, AFF_INFRARED ) || ( !IS_NPC( rch ) && IS_VAMPAFF( rch, VAM_NIGHTSIGHT ) ) ) ) {
...
void do_look( CHAR_DATA *ch, char *argument ) {
	...
	if ( !IS_NPC( ch ) && IS_SET( ch->in_room->room_flags, ROOM_TOTAL_DARKNESS ) && !IS_ITEMAFF( ch, ITEMA_VISION ) && !IS_IMMORTAL( ch ) && !IS_CLASS( ch, CLASS_DROID ) && !( IS_CLASS( ch, CLASS_DROW ) ) ) {
...
void do_who( CHAR_DATA *ch, char *argument ) {
	...
	char kav[256];	  /* Class abbreviation with color codes */
	char faith[64];	  /* Faith indicator, may include xterm color codes */
	char openb[CLASS_SYMBOL_LEN];
	char closeb[CLASS_SYMBOL_LEN];
	...
	for ( d = descriptor_list; d != NULL; d = d->next ) {
		 ...
		 */
		if ( !str_cmp( ch->pcdata->last_decap[0], gch->name ) )
			canDecap = FALSE;
		else
			canDecap = fair_fight( ch, gch );
		if ( !str_cmp( gch->pcdata->last_decap[0], ch->name ) )
			canKillYou = FALSE;
		else
		...
		if ( ( IS_HEAD( gch, LOST_HEAD ) || IS_EXTRA( gch, EXTRA_OSWITCH ) ) && gch->pcdata->chobj != NULL ) {
			if ( gch->pcdata->chobj->pIndexData->vnum == 12 )
				title = " #CA Head#n     #n ";
			else if ( gch->pcdata->chobj->pIndexData->vnum == 30005 )
				title = " #CA Brain#n     ";
			else
				title = " #GAn Object#n   ";
		} else {
			...
			case MAX_LEVEL - 9:
				if ( gch->prefix != NULL )
					...
					title = " #C*#0***A#CF#0K***#C*#n ";
				else if ( IS_SET( gch->pcdata->jflags, JFLAG_BULLY ) )
					title = " #0B#Rully#n       ";
				else if ( mightRate > 3500 )
					title = " #7Almighty#n    ";
		 ...
		 * Symbols are loaded from game.db (class_display table).
		 */
		if ( IS_SCREENREADER( ch ) ) {
			strcpy( openb, "" );
			strcpy( closeb, "" );
		} else {
			const char *sym_left, *sym_right;
			class_get_symbols( gch->class, &sym_left, &sym_right );
			strcpy( openb, sym_left );
			strcpy( closeb, sym_right );
		}

		/*
		 * and then the class name (generation title from game.db).
		 */
		if ( gch->class > 0 ) {
			const char *gen_title = class_get_title( gch->class, gch->generation );
			const char *title_color = class_get_title_color( gch->class );
			char title_buf[128];
			int vis_len, padding;

			/* Build the title with color */
			if ( title_color[0] != '\0' ) {
				/* Simple color prefix + title + reset */
				snprintf( title_buf, sizeof(title_buf), "%s%s#n", title_color, gen_title );
			} else {
				/* Title has embedded colors (e.g., Vampire, Samurai) */
				snprintf( title_buf, sizeof(title_buf), "%s#n", gen_title );
			}

			/* Calculate visible length and add padding to reach ~24 chars total */
			vis_len = visible_strlen( openb ) + visible_strlen( title_buf ) + visible_strlen( closeb );
			padding = ( vis_len < 24 ) ? 24 - vis_len : 1;

			snprintf( kav, sizeof(kav), "%s%s%s%*s", openb, title_buf, closeb, padding, "" );
		} else
			sprintf( kav, "#nNone#n                    " );
...
void do_password( CHAR_DATA *ch, char *argument ) {
	...
	*pArg = '\0';
	...
	if ( strcmp( arg1, ch->pcdata->pwd ) &&
		strcmp( crypt( arg1, ch->name ), ch->pcdata->pwd ) ) {
		WAIT_STATE( ch, 40 );
