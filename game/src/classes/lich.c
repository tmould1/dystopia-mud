...
void do_planeshift( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->pcdata->powers[NECROMANTIC] < cfg( CFG_ABILITY_LICH_PLANESHIFT_LEVEL_REQ ) ) {
...
void do_pentagram( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->pcdata->powers[CON_LORE] < cfg( CFG_ABILITY_LICH_PENTAGRAM_LEVEL_REQ ) ) {
		send_to_char( "I think not.\n\r", ch );
		return;
	}
	if ( ch->mana < cfg( CFG_ABILITY_LICH_PENTAGRAM_MANA_COST ) ) {
...
void do_soulsuck( CHAR_DATA *ch, char *argument ) {
	...
	if ( IS_CLASS( ch, CLASS_LICH ) && ch->pcdata->powers[NECROMANTIC] < cfg( CFG_ABILITY_LICH_SOULSUCK_LEVEL_REQ ) ) {
		send_to_char( "You don't have that power yet.\n\r", ch );
		return;
	}
	...
	WAIT_STATE( ch, cfg( CFG_ABILITY_LICH_SOULSUCK_COOLDOWN ) );
	return;
}
...
void do_earthswallow( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->pcdata->powers[LIFE_LORE] < cfg( CFG_ABILITY_LICH_EARTHSWALLOW_LEVEL_REQ ) ) {
		send_to_char( "You don't have that power yet.\n\r", ch );
		return;
	}
	if ( ch->mana < cfg( CFG_ABILITY_LICH_EARTHSWALLOW_MANA_COST ) ) {
	...
	use_mana( ch, cfg( CFG_ABILITY_LICH_EARTHSWALLOW_MANA_COST ) );
	...
	do_look( mount, "auto" );
	WAIT_STATE( ch, cfg( CFG_ABILITY_LICH_EARTHSWALLOW_COOLDOWN ) );
	return;
}
...
void do_painwreck( CHAR_DATA *ch, char *argument ) {
	...
	argument = one_argument( argument, arg );
	dam = number_range( cfg( CFG_ABILITY_LICH_PAINWRECK_DAMAGE_MIN ), cfg( CFG_ABILITY_LICH_PAINWRECK_DAMAGE_MAX ) );
	...
	if ( ch->pcdata->powers[DEATH_LORE] < cfg( CFG_ABILITY_LICH_PAINWRECK_LEVEL_REQ ) ) {
		send_to_char( "You don't have that power yet.\n\r", ch );
		return;
	}
	if ( ( victim = get_char_room( ch, arg ) ) == NULL ) {
	...
	WAIT_STATE( ch, cfg( CFG_ABILITY_LICH_PAINWRECK_COOLDOWN ) );
	hurt_person( ch, victim, dam );
	...
	if ( number_range( 1, cfg( CFG_ABILITY_LICH_PAINWRECK_STUN_CHANCE ) ) == 1 ) {
...
void do_creepingdoom( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->pcdata->powers[DEATH_LORE] < cfg( CFG_ABILITY_LICH_CREEPINGDOOM_LEVEL_REQ ) ) {
		send_to_char( "You don't have that power yet.\n\r", ch );
		return;
	}
	if ( ch->mana < cfg( CFG_ABILITY_LICH_CREEPINGDOOM_MANA_COST ) ) {
	...
	WAIT_STATE( ch, cfg( CFG_ABILITY_LICH_CREEPINGDOOM_COOLDOWN ) );
	return;
}
...
void do_chillhand( CHAR_DATA *ch, char *argument ) {
	...
	AFFECT_DATA af;
	char arg[MAX_INPUT_LENGTH];
	int dam = number_range( cfg( CFG_ABILITY_LICH_CHILLHAND_DAMAGE_MIN ), cfg( CFG_ABILITY_LICH_CHILLHAND_DAMAGE_MAX ) );
	int sn;
	...
	if ( ch->pcdata->powers[DEATH_LORE] < cfg( CFG_ABILITY_LICH_CHILLHAND_LEVEL_REQ ) ) {
		send_to_char( "You don't have that power yet.\n\r", ch );
		return;
	}
	...
	if ( is_safe( ch, victim ) ) return;
	if ( !IS_NPC( victim ) ) dam /= cfg( CFG_ABILITY_LICH_CHILLHAND_PVP_DAMAGE_DIVISOR );
	damage( ch, victim, dam, gsn_chillhand );
	WAIT_STATE( ch, cfg( CFG_ABILITY_LICH_CHILLHAND_COOLDOWN ) );
	sn = skill_lookup( "chill touch" );
	af.type = sn;
	af.duration = cfg( CFG_ABILITY_LICH_CHILLHAND_DEBUFF_DURATION );
	af.location = APPLY_STR;
	af.modifier = cfg( CFG_ABILITY_LICH_CHILLHAND_STR_MODIFIER );
	af.bitvector = 0;
...
void do_polarity( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->pcdata->powers[LIFE_LORE] < cfg( CFG_ABILITY_LICH_POLARITY_LEVEL_REQ ) ) {
		send_to_char( "You don't have that power yet.\n\r", ch );
		return;
	}
	if ( arg[0] == '\0' && ch->fighting != NULL )
	...
	if ( is_safe( ch, victim ) ) return;
	dam = number_range( cfg( CFG_ABILITY_LICH_POLARITY_DRAIN_MIN ), cfg( CFG_ABILITY_LICH_POLARITY_DRAIN_MAX ) );
	dam = UMIN( dam, victim->mana );
	use_mana( victim, dam );
	heal_char( ch, dam / cfg( CFG_ABILITY_LICH_POLARITY_HEAL_DIVISOR ) );
	...
	WAIT_STATE( ch, cfg( CFG_ABILITY_LICH_POLARITY_COOLDOWN ) );
	return;
}
...
void do_powertransfer( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->mana < cfg( CFG_ABILITY_LICH_POWERTRANSFER_MANA_COST ) ) {
		send_to_char( "You don't have the mana.\n\r", ch );
		return;
	}
	if ( ch->pcdata->powers[LIFE_LORE] < cfg( CFG_ABILITY_LICH_POWERTRANSFER_LEVEL_REQ ) ) {
		send_to_char( "You don't have that power yet.\n\r", ch );
		return;
	}
	...
	if ( has_timer( ch ) ) return;
	heal_char( ch, number_range( cfg( CFG_ABILITY_LICH_POWERTRANSFER_HEAL_MIN ), cfg( CFG_ABILITY_LICH_POWERTRANSFER_HEAL_MAX ) ) );
	use_mana( ch, cfg( CFG_ABILITY_LICH_POWERTRANSFER_MANA_COST ) );
	send_to_char( "You transfer the mystical energies in your body into life energy and heal your wounds.\n\r", ch );
	WAIT_STATE( ch, cfg( CFG_ABILITY_LICH_POWERTRANSFER_COOLDOWN ) );
	return;
}
...
void do_planarstorm( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->mana < cfg( CFG_ABILITY_LICH_PLANARSTORM_MANA_COST ) ) {
		send_to_char( "You can't control the planes.\n\r", ch );
		return;
	}
	if ( ch->pcdata->powers[NECROMANTIC] < cfg( CFG_ABILITY_LICH_PLANARSTORM_LEVEL_REQ ) ) {
		send_to_char( "You don't have that power yet.\n\r", ch );
		return;
	}
...
void do_planartravel( CHAR_DATA *ch, char *argument ) {
	...
	if ( ( victim = get_char_world( ch, arg ) ) == NULL || victim == ch || victim->in_room == NULL || ( !IS_NPC( victim ) && !IS_IMMUNE( victim, IMM_SUMMON ) ) || IS_SET( ch->in_room->room_flags, ROOM_PRIVATE ) || IS_SET( ch->in_room->room_flags, ROOM_SOLITARY ) || IS_SET( ch->in_room->room_flags, ROOM_ASTRAL ) || IS_SET( victim->in_room->room_flags, ROOM_PRIVATE ) || IS_SET( victim->in_room->room_flags, ROOM_SOLITARY ) || IS_SET( victim->in_room->room_flags, ROOM_ASTRAL ) || victim->in_room->vnum == ch->in_room->vnum ) {
		send_to_char( "You failed.\n\r", ch );
		return;
	}
...
void do_summongolem( CHAR_DATA *ch, char *argument ) {
	...
	if ( !str_cmp( arg1, "fire" ) ) {
		...
		victim->level = ch_spl(ch)[RED_MAGIC] / 2;
		victim->hit = 100 * ch_spl(ch)[RED_MAGIC];
		victim->max_hit = 100 * ch_spl(ch)[RED_MAGIC];
		victim->hitroll = ch_spl(ch)[RED_MAGIC];
		victim->damroll = ch_spl(ch)[RED_MAGIC];
		...
		victim->level = ch_spl(ch)[YELLOW_MAGIC] / 2;
		victim->hit = 100 * ch_spl(ch)[YELLOW_MAGIC];
		victim->max_hit = 100 * ch_spl(ch)[YELLOW_MAGIC];
		victim->hitroll = ch_spl(ch)[YELLOW_MAGIC];
		victim->damroll = ch_spl(ch)[YELLOW_MAGIC];
		char_to_room( victim, ch->in_room );
		add_follower( victim, ch );
		af.type = skill_lookup( "charm person" );
		af.duration = 666;
		...
		victim->level = ch_spl(ch)[GREEN_MAGIC] / 2;
		victim->hit = 100 * ch_spl(ch)[GREEN_MAGIC];
		victim->max_hit = 100 * ch_spl(ch)[GREEN_MAGIC];
		victim->hitroll = ch_spl(ch)[GREEN_MAGIC];
		victim->damroll = ch_spl(ch)[GREEN_MAGIC];
		char_to_room( victim, ch->in_room );
		add_follower( victim, ch );
		af.type = skill_lookup( "charm person" );
		af.duration = 666;
		...
		victim->level = ch_spl(ch)[BLUE_MAGIC] / 2;
		victim->hit = 100 * ch_spl(ch)[BLUE_MAGIC];
		victim->max_hit = 100 * ch_spl(ch)[BLUE_MAGIC];
		victim->hitroll = ch_spl(ch)[BLUE_MAGIC];
		victim->damroll = ch_spl(ch)[BLUE_MAGIC];
		char_to_room( victim, ch->in_room );
		add_follower( victim, ch );
		af.type = skill_lookup( "charm person" );
		af.duration = 666;
