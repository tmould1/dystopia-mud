...
void do_stomp( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->pcdata->powers[BULL_LEVEL] < cfg( CFG_ABILITY_SHAPESHIFTER_STOMP_LEVEL_REQ ) ) {
	...
	hurt_person( ch, victim, cfg( CFG_ABILITY_SHAPESHIFTER_STOMP_DAMAGE ) );
	...
	WAIT_STATE( ch, cfg( CFG_ABILITY_SHAPESHIFTER_STOMP_COOLDOWN ) );
	return;
}
...
void do_faeriecurse( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->pcdata->powers[FAERIE_LEVEL] < cfg( CFG_ABILITY_SHAPESHIFTER_FAERIECURSE_LEVEL_REQ ) ) {
	...
	if ( ch->mana < cfg( CFG_ABILITY_SHAPESHIFTER_FAERIECURSE_MANA_COST ) || ch->move < cfg( CFG_ABILITY_SHAPESHIFTER_FAERIECURSE_MOVE_COST ) ) {
	...
	use_mana( ch, cfg( CFG_ABILITY_SHAPESHIFTER_FAERIECURSE_MANA_COST ) );
	use_move( ch, cfg( CFG_ABILITY_SHAPESHIFTER_FAERIECURSE_MOVE_COST ) );
	WAIT_STATE( ch, cfg( CFG_ABILITY_SHAPESHIFTER_FAERIECURSE_COOLDOWN ) );
	return;
}
...
void do_breath( CHAR_DATA *ch, char *argument ) {
	...
	WAIT_STATE( ch, cfg( CFG_ABILITY_SHAPESHIFTER_BREATH_COOLDOWN ) );
	return;
}
...
void do_phase( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->pcdata->powers[TIGER_LEVEL] < cfg( CFG_ABILITY_SHAPESHIFTER_PHASE_LEVEL_REQ ) ) {
	...
	ch->pcdata->powers[PHASE_COUNTER] = cfg( CFG_ABILITY_SHAPESHIFTER_PHASE_DURATION );
...
void do_mistwalk( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->pcdata->powers[SHAPE_POWERS] < cfg( CFG_ABILITY_SHAPESHIFTER_MISTWALK_LEVEL_REQ ) ) {
	...
	if ( ch->move < cfg( CFG_ABILITY_SHAPESHIFTER_MISTWALK_MOVE_COST ) ) {
	...
	use_move( ch, cfg( CFG_ABILITY_SHAPESHIFTER_MISTWALK_MOVE_COST ) );
	char_from_room( ch );
...
void do_hatform( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->pcdata->powers[SHAPE_POWERS] < cfg( CFG_ABILITY_SHAPESHIFTER_HATFORM_LEVEL_REQ ) ) {
...
void do_shift( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->pcdata->powers[SHAPE_COUNTER] > cfg( CFG_ABILITY_SHAPESHIFTER_SHIFT_COUNTER_MAX ) ) {
		send_to_char( "You're too tired from all this formshifting.\n\r", ch );
		return;
	}
	if ( !str_cmp( arg, "tiger" ) ) {
		...
		ch->pcdata->powers[SHAPE_COUNTER] += cfg( CFG_ABILITY_SHAPESHIFTER_SHIFT_COUNTER_INCREMENT );
		SET_BIT( ch->affected_by, AFF_POLYMORPH );
		ch->damroll += cfg( CFG_ABILITY_SHAPESHIFTER_SHIFT_TIGER_DAMROLL );
		ch->hitroll += cfg( CFG_ABILITY_SHAPESHIFTER_SHIFT_TIGER_HITROLL );
		...
		ch->pcdata->powers[SHAPE_COUNTER] += cfg( CFG_ABILITY_SHAPESHIFTER_SHIFT_COUNTER_INCREMENT );
		SET_BIT( ch->affected_by, AFF_POLYMORPH );
		ch->damroll += cfg( CFG_ABILITY_SHAPESHIFTER_SHIFT_HYDRA_DAMROLL );
		ch->hitroll += cfg( CFG_ABILITY_SHAPESHIFTER_SHIFT_HYDRA_HITROLL );
		...
		ch->pcdata->powers[SHAPE_COUNTER] += cfg( CFG_ABILITY_SHAPESHIFTER_SHIFT_COUNTER_INCREMENT );
		SET_BIT( ch->affected_by, AFF_POLYMORPH );
		ch->damroll += cfg( CFG_ABILITY_SHAPESHIFTER_SHIFT_BULL_DAMROLL );
		ch->hitroll += cfg( CFG_ABILITY_SHAPESHIFTER_SHIFT_BULL_HITROLL );
		...
		ch->pcdata->powers[SHAPE_COUNTER] += cfg( CFG_ABILITY_SHAPESHIFTER_SHIFT_COUNTER_INCREMENT );
		SET_BIT( ch->affected_by, AFF_POLYMORPH );
		ch->damroll += cfg( CFG_ABILITY_SHAPESHIFTER_SHIFT_FAERIE_DAMROLL );
		ch->hitroll += cfg( CFG_ABILITY_SHAPESHIFTER_SHIFT_FAERIE_HITROLL );
...
void do_formlearn( CHAR_DATA *ch, char *argument ) {
	...
	if ( !str_cmp( arg, "hydra" ) ) {
		if ( ch->pcdata->powers[HYDRA_LEVEL] >= cfg( CFG_ABILITY_SHAPESHIFTER_FORMLEARN_MAX_LEVEL ) ) {
			send_to_char( "Your Hydra form is complete.\n\r", ch );
			return;
		}
		if ( ch->practice < ( cfg( CFG_ABILITY_SHAPESHIFTER_FORMLEARN_COST_MULTIPLIER ) * ch->pcdata->powers[HYDRA_LEVEL] + cfg( CFG_ABILITY_SHAPESHIFTER_FORMLEARN_COST_MULTIPLIER ) ) ) {
		...
		ch->practice -= ch->pcdata->powers[HYDRA_LEVEL] * cfg( CFG_ABILITY_SHAPESHIFTER_FORMLEARN_COST_MULTIPLIER );
		return;
