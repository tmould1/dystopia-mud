...
void do_stalk( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->move < cfg( CFG_ABILITY_NINJA_STALK_MOVE_COST ) ) {
	...
	use_move( ch, cfg( CFG_ABILITY_NINJA_STALK_MOVE_COST ) );
...
void do_principles( CHAR_DATA *ch, char *argument ) {
	...
	if ( !str_cmp( arg2, "improve" ) ) {
		...
		if ( !str_cmp( arg1, "sora" ) ) {
			improve = NPOWER_SORA;
			max = cfg( CFG_ABILITY_NINJA_PRINCIPLES_SORA_MAX );
		} else if ( !str_cmp( arg1, "chikyu" ) ) {
			improve = NPOWER_CHIKYU;
			max = cfg( CFG_ABILITY_NINJA_PRINCIPLES_CHIKYU_MAX );
		} else if ( !str_cmp( arg1, "ningenno" ) ) {
			improve = NPOWER_NINGENNO;
			max = cfg( CFG_ABILITY_NINJA_PRINCIPLES_NINGENNO_MAX );
		} else {
...
void do_michi( CHAR_DATA *ch, char *argument ) {
	...
	if ( IS_CLASS( ch, CLASS_NINJA ) ) {
		...
		if ( ch->rage >= cfg( CFG_ABILITY_NINJA_MICHI_RAGE_THRESHOLD ) ) {
			send_to_char( "But you are already in the state of Michi.\n\r", ch );
			return;
		}
		if ( ch->move < cfg( CFG_ABILITY_NINJA_MICHI_MOVE_COST ) ) {
		...
		ch->rage += cfg( CFG_ABILITY_NINJA_MICHI_RAGE_GAIN );
		use_move( ch, cfg( CFG_ABILITY_NINJA_MICHI_MOVE_COST ) );
		WAIT_STATE( ch, cfg( CFG_ABILITY_NINJA_MICHI_COOLDOWN ) );
		return;
	} else
		send_to_char( "But you are already in the state of Michi.\n\r", ch );
	return;
...
void do_kakusu( CHAR_DATA *ch, char *argument ) {
	...
	if ( IS_CLASS( ch, CLASS_NINJA ) && ch->pcdata->powers[NPOWER_SORA] < cfg( CFG_ABILITY_NINJA_KAKUSU_LEVEL_REQ ) ) {
		send_to_char( "You have not learned the Sora principle to 3.\n\r", ch );
		return;
	}

	if ( ch->move < cfg( CFG_ABILITY_NINJA_KAKUSU_MOVE_COST ) ) {
	...
	if ( IS_SET( ch->act, AFF_HIDE ) ) {
		...
		use_move( ch, cfg( CFG_ABILITY_NINJA_KAKUSU_MOVE_COST ) );
...
void do_kanzuite( CHAR_DATA *ch, char *argument ) {
	...
	if ( IS_CLASS( ch, CLASS_NINJA ) && ch->pcdata->powers[NPOWER_SORA] < cfg( CFG_ABILITY_NINJA_KANZUITE_LEVEL_REQ ) ) {
		send_to_char( "You have not learned the Sora principle to 5.\n\r", ch );
		return;
	}

	else if ( ch->move < cfg( CFG_ABILITY_NINJA_KANZUITE_MOVE_COST ) ) {
	...
	if ( IS_SET( ch->act, PLR_HOLYLIGHT ) ) {
		...
		use_move( ch, cfg( CFG_ABILITY_NINJA_KANZUITE_MOVE_COST ) );
	}
...
void do_mienaku( CHAR_DATA *ch, char *argument ) {
	...
	if ( IS_CLASS( ch, CLASS_NINJA ) && ch->pcdata->powers[NPOWER_NINGENNO] < cfg( CFG_ABILITY_NINJA_MIENAKU_LEVEL_REQ ) ) {
	...
	if ( ch->move < cfg( CFG_ABILITY_NINJA_MIENAKU_MOVE_COST ) ) {
...

	{
		...
		for ( attempt = 0; attempt < 6; attempt++ ) {
			...
			/*	if (victim->in_room == ch->in_room)
			  ...
			  */
			use_move( ch, cfg( CFG_ABILITY_NINJA_MIENAKU_MOVE_COST ) );
...
void do_bomuzite( CHAR_DATA *ch, char *argument ) {
	...
	if ( !IS_CLASS( ch, CLASS_NINJA ) ) {
		send_to_char( "Huh?\n\r", ch );
		return;
	} else if ( ch->pcdata->powers[NPOWER_SORA] < cfg( CFG_ABILITY_NINJA_BOMUZITE_LEVEL_REQ ) ) {
	...
	if ( ch->move < cfg( CFG_ABILITY_NINJA_BOMUZITE_MOVE_COST ) ) {
	...
	if ( victim->in_room == ch->in_room ) {
		...
		use_move( ch, cfg( CFG_ABILITY_NINJA_BOMUZITE_MOVE_COST ) );
		WAIT_STATE( ch, cfg( CFG_ABILITY_NINJA_BOMUZITE_COOLDOWN ) );
		return;
	}
...
void do_tsume( CHAR_DATA *ch, char *argument ) {
	...
	if ( !IS_CLASS( ch, CLASS_NINJA ) || ch->pcdata->powers[NPOWER_NINGENNO] < cfg( CFG_ABILITY_NINJA_TSUME_LEVEL_REQ ) ) {
		send_to_char( "Huh?\n\r", ch );
		return;
	}
...
void do_hara_kiri( CHAR_DATA *ch, char *argument ) {
	...
	if ( !IS_CLASS( ch, CLASS_NINJA ) || ch->pcdata->powers[NPOWER_CHIKYU] < cfg( CFG_ABILITY_NINJA_HARA_KIRI_LEVEL_REQ ) ) {
		send_to_char( "Huh?\n\r", ch );
		return;
	}
	...
	if ( ch->pcdata->powers[HARA_KIRI] < cfg( CFG_ABILITY_NINJA_HARA_KIRI_MIN_DURATION ) )
		ch->pcdata->powers[HARA_KIRI] = cfg( CFG_ABILITY_NINJA_HARA_KIRI_MIN_DURATION );
	ch->hit = 1;
	ch->mana = 1;
	ch->move = 1;
...
void do_strangle( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->pcdata->powers[NPOWER_NINGENNO] < cfg( CFG_ABILITY_NINJA_STRANGLE_LEVEL_REQ ) ) {
	...
	if ( chance > cfg( CFG_ABILITY_NINJA_STRANGLE_FAIL_THRESHOLD ) ) {
		int i;
		for ( i = 0; i < cfg( CFG_ABILITY_NINJA_STRANGLE_HITS_SUCCESS ); i++ )
			one_hit( ch, victim, gsn_backstab, 1 );
		WAIT_STATE( ch, cfg( CFG_ABILITY_NINJA_STRANGLE_COOLDOWN_SUCCESS ) );
	} else {
		send_to_char( "You failed your attempt.\n\r", ch );
		one_hit( ch, victim, gsn_backstab, 1 );
		WAIT_STATE( ch, cfg( CFG_ABILITY_NINJA_STRANGLE_COOLDOWN_FAIL ) );
	}
	return;
}
