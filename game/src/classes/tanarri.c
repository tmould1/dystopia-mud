...
void do_taneq( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->practice < cfg( CFG_ABILITY_TANARRI_TANEQ_PRIMAL_COST ) ) {
	...
	obj_to_char( obj, ch );
	ch->practice -= cfg( CFG_ABILITY_TANARRI_TANEQ_PRIMAL_COST );
...
void do_chaossurge( CHAR_DATA *ch, char *argument ) {
	...
	if ( victim->alignment > 500 )
		dam = cfg( CFG_ABILITY_TANARRI_CHAOSSURGE_DAMAGE_GOOD );
...
void do_enmity( CHAR_DATA *ch, char *argument ) {
	...
	if ( number_percent() > 60 )
		...
		send_to_char( "#RSomeone is trying to control your actions!!!#n\n\r", victim2 );
	WAIT_STATE( ch, cfg( CFG_ABILITY_TANARRI_ENMITY_COOLDOWN ) );
	return;
}
...
void do_enrage( CHAR_DATA *ch, char *argument ) {
	...
	if ( number_percent() > 40 )
		...
		send_to_char( "#RSomeone is trying to control your actions!!!#n\n\r", victim );
	WAIT_STATE( ch, cfg( CFG_ABILITY_TANARRI_ENRAGE_COOLDOWN ) );
	return;
}
...
void do_lavablast( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->mana < cfg( CFG_ABILITY_TANARRI_LAVABLAST_MANA_COST ) || ch->move < cfg( CFG_ABILITY_TANARRI_LAVABLAST_MOVE_COST ) ) {
	...
	WAIT_STATE( ch, cfg( CFG_ABILITY_TANARRI_LAVABLAST_COOLDOWN ) );
	return;
}
...
void do_chaosgate( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->move < cfg( CFG_ABILITY_TANARRI_CHAOSGATE_MOVE_COST ) ) {
	...
	use_move( ch, cfg( CFG_ABILITY_TANARRI_CHAOSGATE_MOVE_COST ) );
	char_from_room( ch );
...
void do_booming( CHAR_DATA *ch, char *argument ) {
	...
	WAIT_STATE( ch, cfg( CFG_ABILITY_TANARRI_BOOMING_COOLDOWN ) );
	return;
}
...
void do_fury( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->pcdata->powers[TANARRI_FURY_ON] == 1 ) {
		ch->hitroll -= cfg( CFG_ABILITY_TANARRI_FURY_HITROLL_BONUS );
		ch->damroll -= cfg( CFG_ABILITY_TANARRI_FURY_DAMROLL_BONUS );
...
void do_earthquake( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->mana < cfg( CFG_ABILITY_TANARRI_EARTHQUAKE_MANA_COST ) ) {
...
void do_tornado( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->mana < cfg( CFG_ABILITY_TANARRI_TORNADO_MANA_COST ) ) {
...
void do_infernal( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->mana < cfg( CFG_ABILITY_TANARRI_INFERNAL_MANA_COST ) ) {
		if ( IS_CLASS( ch, CLASS_TANARRI ) )
