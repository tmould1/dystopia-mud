...
void do_spiritform( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->pcdata->powers[ANGEL_PEACE] < cfg( CFG_ABILITY_ANGEL_SPIRITFORM_LEVEL_REQ ) ) {
...
void do_gpeace( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->pcdata->powers[ANGEL_PEACE] < cfg( CFG_ABILITY_ANGEL_GPEACE_LEVEL_REQ ) ) {
		send_to_char( "Your not peaceful enough.\n\r", ch );
		return;
	}
	if ( has_timer( ch ) ) return;
...
void do_innerpeace( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->pcdata->powers[ANGEL_PEACE] < cfg( CFG_ABILITY_ANGEL_INNERPEACE_LEVEL_REQ ) ) {
		send_to_char( "Your not peaceful enough.\n\r", ch );
		return;
	}
	if ( ch->mana < cfg( CFG_ABILITY_ANGEL_INNERPEACE_MANA_COST ) ) {
		send_to_char( "You don't have enough mana.\n\r", ch );
		return;
	}
	use_mana( ch, cfg( CFG_ABILITY_ANGEL_INNERPEACE_MANA_COST ) );
	heal_char( ch, ch->pcdata->powers[ANGEL_PEACE] * cfg( CFG_ABILITY_ANGEL_INNERPEACE_HEAL_MULTIPLIER ) );
	...
	WAIT_STATE( ch, cfg( CFG_ABILITY_ANGEL_INNERPEACE_COOLDOWN ) );
	return;
}
...
void do_houseofgod( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->pcdata->powers[ANGEL_PEACE] < cfg( CFG_ABILITY_ANGEL_HOUSEOFGOD_LEVEL_REQ ) ) {
		send_to_char( "Your not peaceful enough.\n\r", ch );
		return;
	}
	if ( has_timer( ch ) ) return;
	...
	ch->pcdata->powers[ANGEL_PEACE_COUNTER] = cfg( CFG_ABILITY_ANGEL_HOUSEOFGOD_PEACE_COUNTER );
	ch->level = 12;
	do_peace( ch, "" );
	ch->level = 3;
	...
	WAIT_STATE( ch, cfg( CFG_ABILITY_ANGEL_HOUSEOFGOD_COOLDOWN ) );
	return;
}
...
void do_angelicaura( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->pcdata->powers[ANGEL_HARMONY] < cfg( CFG_ABILITY_ANGEL_ANGELICAURA_LEVEL_REQ ) ) {
...
void do_gbanish( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->pcdata->powers[ANGEL_HARMONY] < cfg( CFG_ABILITY_ANGEL_GBANISH_LEVEL_REQ ) ) {
		send_to_char( "Your not in contact with your inner harmonies.\n\r", ch );
		return;
	}
	...
	if ( victim->alignment > 0 )
		dam = cfg( CFG_ABILITY_ANGEL_GBANISH_DAM_GOOD );
	...
	WAIT_STATE( ch, cfg( CFG_ABILITY_ANGEL_GBANISH_COOLDOWN ) );
	return;
}
...
void do_harmony( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->pcdata->powers[ANGEL_HARMONY] < cfg( CFG_ABILITY_ANGEL_HARMONY_LEVEL_REQ ) ) {
		send_to_char( "Your not in contact with your inner harmonies.\n\r", ch );
		return;
	}
	if ( ( victim = get_char_room( ch, arg ) ) == NULL ) {
	...
	WAIT_STATE( ch, cfg( CFG_ABILITY_ANGEL_HARMONY_COOLDOWN ) );
	return;
}
...
void do_gsenses( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->pcdata->powers[ANGEL_LOVE] < cfg( CFG_ABILITY_ANGEL_GSENSES_LEVEL_REQ ) ) {
...
void do_gfavor( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->pcdata->powers[ANGEL_LOVE] < cfg( CFG_ABILITY_ANGEL_GFAVOR_LEVEL_REQ ) ) {
	...
	if ( ch->move < cfg( CFG_ABILITY_ANGEL_GFAVOR_MOVE_COST ) || ch->mana < cfg( CFG_ABILITY_ANGEL_GFAVOR_MANA_COST ) ) {
...
void do_forgivness( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->pcdata->powers[ANGEL_LOVE] < cfg( CFG_ABILITY_ANGEL_FORGIVNESS_LEVEL_REQ ) ) {
	...
	heal_char( victim, number_range( cfg( CFG_ABILITY_ANGEL_FORGIVNESS_HEAL_MIN ), cfg( CFG_ABILITY_ANGEL_FORGIVNESS_HEAL_MAX ) ) );
	...
	WAIT_STATE( ch, cfg( CFG_ABILITY_ANGEL_FORGIVNESS_COOLDOWN ) );
	return;
}
...
void do_martyr( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->pcdata->powers[ANGEL_LOVE] < cfg( CFG_ABILITY_ANGEL_MARTYR_LEVEL_REQ ) ) {
	...
	ch->move = 1;
	ch->mana = 1;
	WAIT_STATE( ch, cfg( CFG_ABILITY_ANGEL_MARTYR_COOLDOWN ) );
	return;
}
...
void do_swoop( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->pcdata->powers[ANGEL_JUSTICE] < cfg( CFG_ABILITY_ANGEL_SWOOP_LEVEL_REQ ) ) {
	...
	if ( ch->move < cfg( CFG_ABILITY_ANGEL_SWOOP_MOVE_COST ) ) {
	...
	use_move( ch, cfg( CFG_ABILITY_ANGEL_SWOOP_MOVE_COST ) );
	char_from_room( ch );
...
void do_touchofgod( CHAR_DATA *ch, char *argument ) {
	...
	argument = one_argument( argument, arg );
	dam = number_range( cfg( CFG_ABILITY_ANGEL_TOUCHOFGOD_DAM_MIN ), cfg( CFG_ABILITY_ANGEL_TOUCHOFGOD_DAM_MAX ) );
	...
	if ( ch->pcdata->powers[ANGEL_JUSTICE] < cfg( CFG_ABILITY_ANGEL_TOUCHOFGOD_LEVEL_REQ ) ) {
	...
	WAIT_STATE( ch, cfg( CFG_ABILITY_ANGEL_TOUCHOFGOD_COOLDOWN ) );
	hurt_person( ch, victim, dam );
...
void do_awings( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->pcdata->powers[ANGEL_JUSTICE] < cfg( CFG_ABILITY_ANGEL_AWINGS_LEVEL_REQ ) ) {
...
void do_halo( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->pcdata->powers[ANGEL_JUSTICE] < cfg( CFG_ABILITY_ANGEL_HALO_LEVEL_REQ ) ) {
...
void do_sinsofthepast( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->pcdata->powers[ANGEL_JUSTICE] < cfg( CFG_ABILITY_ANGEL_SINSOFTHEPAST_LEVEL_REQ ) ) {
	...
	WAIT_STATE( ch, cfg( CFG_ABILITY_ANGEL_SINSOFTHEPAST_COOLDOWN ) );
	return;
}
...
void do_angelicarmor( CHAR_DATA *ch, char *argument ) {
	do_classarmor_generic( ch, argument, CLASS_ANGEL );
}
