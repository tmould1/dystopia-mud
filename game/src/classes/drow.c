...
void do_chaosblast( CHAR_DATA *ch, char *argument ) {
	...
	if ( !IS_CLASS( ch, CLASS_DROW ) || ( !IS_SET( ch->special, SPC_DROW_MAG ) && ch->generation > 2 ) ) {
		return;
	}
...
void do_darkness( CHAR_DATA *ch, char *argument ) {
	...
	if ( ( !IS_CLASS( ch, CLASS_DROW ) && !IS_CLASS( ch, CLASS_DROID ) ) || ( !IS_SET( ch->pcdata->powers[1], DPOWER_DARKNESS ) && !IS_CLASS( ch, CLASS_DROID ) ) ) {
