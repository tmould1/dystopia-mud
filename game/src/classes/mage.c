...
void do_reveal( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->mana < cfg( CFG_ABILITY_MAGE_REVEAL_MANA_COST ) ) {
	...
	use_mana( ch, cfg( CFG_ABILITY_MAGE_REVEAL_MANA_COST ) );
	return;
}

void do_magearmor( CHAR_DATA *ch, char *argument ) {
	do_classarmor_generic( ch, argument, CLASS_MAGE );
}
...
void do_chaosmagic( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->mana < cfg( CFG_ABILITY_MAGE_CHAOSMAGIC_MANA_COST ) ) {
	...
	use_mana( ch, cfg( CFG_ABILITY_MAGE_CHAOSMAGIC_MANA_COST ) );
	WAIT_STATE( ch, cfg( CFG_ABILITY_MAGE_CHAOSMAGIC_COOLDOWN ) );
	return;
}
...
void do_chant( CHAR_DATA *ch, char *argument ) {
	...
	if ( !str_cmp( arg1, "heal" ) ) {
		...
		if ( ch->mana < cfg( CFG_ABILITY_MAGE_CHANT_HEAL_MANA_COST ) ) {
		...
		WAIT_STATE( ch, cfg( CFG_ABILITY_MAGE_CHANT_HEAL_COOLDOWN ) );
		use_mana( ch, cfg( CFG_ABILITY_MAGE_CHANT_HEAL_MANA_COST ) );
		...
		if ( ch->mana < cfg( CFG_ABILITY_MAGE_CHANT_DAMAGE_MANA_COST ) ) {
		...
		WAIT_STATE( ch, cfg( CFG_ABILITY_MAGE_CHANT_DAMAGE_COOLDOWN ) );
		use_mana( ch, cfg( CFG_ABILITY_MAGE_CHANT_DAMAGE_MANA_COST ) );
		...
		if ( ch->mana < cfg( CFG_ABILITY_MAGE_CHANT_BLESS_MANA_COST ) ) {
		...
		if ( ch->mana < cfg( CFG_ABILITY_MAGE_CHANT_CURSE_MANA_COST ) ) {
			send_to_char( "You need 2500 mana.\n\r", ch );
			return;
		}
		if ( victim->level < 3 ) {
...
void do_invoke( CHAR_DATA *ch, char *argument ) {
	...
	int shieldcost = cfg( CFG_ABILITY_MAGE_INVOKE_MAGESHIELD_PRACTICE_COST );
	int steelshieldcost = cfg( CFG_ABILITY_MAGE_INVOKE_STEELSHIELD_PRACTICE_COST );
	int beastcost = cfg( CFG_ABILITY_MAGE_INVOKE_BEAST_PRACTICE_COST );
	int illusioncost = cfg( CFG_ABILITY_MAGE_INVOKE_ILLUSIONS_PRACTICE_COST );
	int defshieldcost = cfg( CFG_ABILITY_MAGE_INVOKE_DEFLECTOR_PRACTICE_COST );
	...
	if ( !str_cmp( arg1, "learn" ) ) {
		int cost = ( ch->pcdata->powers[PINVOKE] + 1 ) * cfg( CFG_ABILITY_MAGE_INVOKE_LEARN_COST_MULTIPLIER );
		if ( ch->pcdata->powers[PINVOKE] >= cfg( CFG_ABILITY_MAGE_INVOKE_LEARN_MAX_LEVEL ) ) {
		...
		if ( ch->pcdata->powers[PINVOKE] < cfg( CFG_ABILITY_MAGE_INVOKE_STEELSHIELD_LEVEL_REQ ) ) {
		...
		if ( ch->pcdata->powers[PINVOKE] < cfg( CFG_ABILITY_MAGE_INVOKE_BEAST_LEVEL_REQ ) ) {
			send_to_char( "You don't have that invoke yet.\n\r", ch );
			return;
		}
		...
		if ( ch->pcdata->powers[PINVOKE] < cfg( CFG_ABILITY_MAGE_INVOKE_ILLUSIONS_LEVEL_REQ ) ) {
			send_to_char( "You don't have that invoke yet.\n\r", ch );
			return;
		}
		...
		if ( ch->pcdata->powers[PINVOKE] < cfg( CFG_ABILITY_MAGE_INVOKE_DEFLECTOR_LEVEL_REQ ) ) {
			send_to_char( "You don't have that invoke yet.\n\r", ch );
			return;
		}
		...
		if ( ch->pcdata->powers[PINVOKE] < cfg( CFG_ABILITY_MAGE_INVOKE_MAGESHIELD_LEVEL_REQ ) ) {
			send_to_char( "You don't have that invoke yet.\n\r", ch );
			return;
		}
		...
		int totalcost = 0;
		if ( ch->pcdata->powers[PINVOKE] < cfg( CFG_ABILITY_MAGE_INVOKE_ALL_LEVEL_REQ ) ) {
...
void do_discharge( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->pcdata->powers[PINVOKE] < cfg( CFG_ABILITY_MAGE_DISCHARGE_LEVEL_REQ ) ) /* 4 is just a test */
	{
		send_to_char( "You don't have that invoke yet.\n\r", ch );
		return;
	}
...
void do_teleport( CHAR_DATA *ch, char *argument ) {
	...
	if ( ch->pcdata->powers[PINVOKE] < cfg( CFG_ABILITY_MAGE_TELEPORT_LEVEL_REQ ) ) /* 1 is just a test */
	{
		send_to_char( "You don't have that invoke yet.\n\r", ch );
		return;
	}
	...
	if ( ch->mana < cfg( CFG_ABILITY_MAGE_TELEPORT_MANA_COST ) ) {
	...
	use_mana( ch, cfg( CFG_ABILITY_MAGE_TELEPORT_MANA_COST ) );
	char_from_room( ch );
