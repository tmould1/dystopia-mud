...
static void mxp_build_inventory_menu( OBJ_DATA *obj, CHAR_DATA *ch,
	...
	{
		bool is_metal = ( obj->item_type == ITEM_COPPER || obj->item_type == ITEM_IRON || obj->item_type == ITEM_STEEL || obj->item_type == ITEM_ADAMANTITE );
		...
		if ( ( is_metal || is_gem || is_hilt ) && ch->carrying != NULL ) {
			...
			for ( cont = ch->carrying; cont != NULL && forge_count < max_forge_targets; cont = cont->next_content ) {
				...
				if ( is_metal && ( IS_SET( cont->spectype, SITEM_COPPER ) || IS_SET( cont->spectype, SITEM_IRON ) || IS_SET( cont->spectype, SITEM_STEEL ) || IS_SET( cont->spectype, SITEM_ADAMANTITE ) ) )
					continue;
...
char *mxp_exit_link( EXIT_DATA *pexit, int door, CHAR_DATA *ch, char *display_text ) {
	...
	} else if ( room_is_dark( to_room ) && !IS_AFFECTED( ch, AFF_INFRARED ) && !IS_SET( ch->act, PLR_HOLYLIGHT ) ) {
