...
void do_racecommands( CHAR_DATA *ch, char *argument ) {
	...
	displayed = FALSE;
	for ( cmd = 0; cmd_table[cmd].name[0] != '\0'; cmd++ ) {
		if ( IS_CLASS( ch, cmd_table[cmd].race ) && cmd_table[cmd].discipline == 0 ) {
			strcpy( buf, "" );
			if ( i == 0 ) {
				switch ( ch->class ) {
				case 1:
...
const struct cmd_type cmd_table[] =
	{
		...
		{ "buy", do_buy, POS_STANDING, 0, LOG_NORMAL, 0, 0, 0 },
		{ "sell", do_sell, POS_STANDING, 0, LOG_NORMAL, 0, 0, 0 },
		{ "list", do_list, POS_STANDING, 0, LOG_NORMAL, 0, 0, 0 },
		{ "value", do_value, POS_STANDING, 0, LOG_NORMAL, 0, 0, 0 },
		...
		{ "pretitle", do_pretitle, POS_DEAD, 12, LOG_ALWAYS, 0, 0, 0 },
		...
		{ "pedit", do_pedit, POS_DEAD, 7, LOG_ALWAYS, 0, 0, 0 },
